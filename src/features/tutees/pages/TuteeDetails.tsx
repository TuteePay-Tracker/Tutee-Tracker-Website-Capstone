...
import { Tutee, ScheduleItem, GRADE_LEVELS, getTuteeYearTotals } from '@/features/tutees/types/tutee';
...
export const TuteeDetails = () => {
  ...
  const [scheduleTime, setScheduleTime] = useState({ startTime: '09:00', endTime: '10:00' });

  const getInitialScheduleFromTutee = (currentTutee?: Tutee): ScheduleItem[] => {
    if (!currentTutee?.schedule) return [];
    if (Array.isArray(currentTutee.schedule)) {
      return currentTutee.schedule.map((item: any) => {
        if (typeof item === 'object' && item !== null && 'startTime' in item && 'endTime' in item) {
          return item;
        } else if (typeof item === 'object' && item !== null && 'time' in item) {
          return { day: item.day, startTime: item.time, endTime: item.time };
        }
        return { day: String(item), startTime: '09:00', endTime: '10:00' };
      });
    } else if (typeof currentTutee.schedule === 'string') {
      const parts = (currentTutee.schedule as string).split(/\r?\n|,/);
      return parts.map(p => p.trim()).filter(Boolean).map(day => ({
        day,
        startTime: '09:00',
        endTime: '10:00'
      }));
  ...
  const getInitialTimeFromTutee = (currentTutee?: Tutee) => {
    if (currentTutee && Array.isArray(currentTutee.schedule) && currentTutee.schedule.length > 0) {
      const first = currentTutee.schedule[0];
      if (typeof first === 'object' && first !== null && 'startTime' in first && 'endTime' in first) {
  ...
  const toggleEditDay = (day: string) => {
    const existingIndex = editForm.schedule.findIndex(s => s.day === day);
    if (existingIndex >= 0) {
      setEditForm({
        ...editForm,
        schedule: editForm.schedule.filter((_, i) => i !== existingIndex),
      });
    } else {
      setEditForm({
        ...editForm,
  ...
  const isEditDaySelected = (day: string) => {
    return editForm.schedule.some(s => s.day === day);
  };

  const updateEditScheduleTimes = (startTime: string, endTime: string) => {
  ...
  const handleStartEdit = () => {
    if (!tutee) return;
    const initialSchedule = getInitialScheduleFromTutee(tutee);
    const initialTime = getInitialTimeFromTutee(tutee);
    setEditForm({
  ...
  const handleSaveInfo = async () => {
    ...
    try {
      ...
      await dayPaymentService.syncTuteeScheduleRecords(tutee.id, updates.schedule);
      ...
      if (user) {
        await logActivity(
          ...
          'Student Updated',
          'Students',
          `Updated personal information and schedule for student ${updates.firstName} ${updates.surname}`
        );
      }
    } catch (error) {
  ...
  // Load payments and attendance when tutee changes or school year changes
  useEffect(() => {
  ...
  const loadAttendance = () => {
    ...
    return dayPaymentService.subscribeToRecordsByTutee(
      tutee.id,
      (data) => {
        setAttendanceRecords(data.filter((r) => belongsToSchoolYear(selectedYear, r)));
  ...
  // Totals are scoped to the selected school year via totalsByYear when available.
  const yearTotals = getTuteeYearTotals(tutee, selectedYear);
  const totalPaid = Math.round((yearTotals.totalPaid || 0) * 100) / 100;
  const remainingBalance = Math.max(Math.round((yearTotals.balance || 0) * 100) / 100, 0);
  ...
  const isFull = (yearTotals.totalSessions || 0) > 0 && !hasOutstandingBalance;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {isEditingInfo ? (
                <div className="space-y-3.5">
                  ...
                  <div className="border border-gray-200 rounded-xl p-3 bg-gray-50/50 space-y-3">
                    <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wide">Schedule *</label>

                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 mb-1">Class Start</label>
                        <input
                          type="time"
                          value={scheduleTime.startTime}
                          onChange={(e) => updateEditScheduleTimes(e.target.value, scheduleTime.endTime)}
                          className="w-full p-2 border border-gray-200 rounded-lg text-xs bg-white focus:ring-2 focus:ring-green-700 outline-none"
                        />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 mb-1">Class End</label>
                        <input
                          type="time"
                          value={scheduleTime.endTime}
                          onChange={(e) => updateEditScheduleTimes(scheduleTime.startTime, e.target.value)}
                          className="w-full p-2 border border-gray-200 rounded-lg text-xs bg-white focus:ring-2 focus:ring-green-700 outline-none"
                        />
                      </div>
                    </div>
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-gray-500 mb-1.5">Select Days</label>
                      <div className="grid grid-cols-2 gap-1.5">
                        {DAYS.map(day => {
                          const isSelected = isEditDaySelected(day);
                          return (
                            <button
                              key={day}
                              type="button"
                    ...
                    {editForm.schedule.length > 0 && (
                      <div className="text-xs text-green-800 bg-green-50 border border-green-200 p-2 rounded-lg">
                        <strong>Selected:</strong> {editForm.schedule.map(s => s.day).join(', ')} ({formatTime12h(scheduleTime.startTime)} - {formatTime12h(scheduleTime.endTime)})
                      </div>
                    )}
                  </div>
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <div className="flex justify-between pb-3 border-b border-gray-100 text-sm text-gray-600">
                    ...
                    <span className="font-semibold text-gray-900">{yearTotals.totalSessions}</span>
                  </div>
                  <div className="flex justify-between pb-3 border-b border-gray-100 text-sm text-gray-600">
                ...
                <div className="space-y-4">
                  ...
                  {yearTotals.lastPaymentDate && (
                    <div className="text-xs text-gray-500 bg-gray-50 rounded-xl p-3 border border-gray-100 mt-2">
                      Last payment recorded on: <strong>{formatDate(yearTotals.lastPaymentDate)}</strong>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="bg-gradient-to-br from-green-700 to-green-900 rounded-2xl p-6 text-white shadow-lg shadow-green-700/10">
              ...
              <p className="text-4xl font-extrabold">{yearTotals.totalSessions}</p>
